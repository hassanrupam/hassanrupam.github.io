...
export class Chrome extends Component {
    ...
    componentDidMount() {
        let lastVisitedUrl = secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.CHROME_URL);
        let lastDisplayedUrl = secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.CHROME_DISPLAY_URL);
    ...
    storeVisitedUrl = (url, display_url) => {
        secureLocalStorage.setItem(SECURE_STORAGE_STORE_KEY.CHROME_URL, url);
        secureLocalStorage.setItem(SECURE_STORAGE_STORE_KEY.CHROME_DISPLAY_URL, display_url);
    }

    refreshChrome = () => {
        document.getElementById(APPLICATION_UNIQUE_ID.CHROME_SCREEN).src += '';
    }
    ...
    render() {
        return (
            <div className="h-full w-full flex flex-col bg-ub-cool-grey">
                {this.displayUrlBar()}
                <iframe src={this.state.url} className="flex-grow" id={APPLICATION_UNIQUE_ID.CHROME_SCREEN} frameBorder="0" title="Ubuntu Chrome Url"></iframe>
            </div>
        )
    }
}
