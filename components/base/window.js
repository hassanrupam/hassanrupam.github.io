...
export class Window extends Component {
    constructor() {
        ...
        this.state = {
            ...
            width: 60,
            height: 85,
            closed: APP_CONSTANTS.BOOLEAN.FALSE,
            maximized: APP_CONSTANTS.BOOLEAN.FALSE,
            parentSize: {
                height: 100,
                width: 100
            }
    ...
    componentDidMount() {
        ...
        window.addEventListener(APPLICATION_EVENTS.RESIZE, this.resizeBoundries);
    }

    componentWillUnmount() {
        // ReactGA.pageview("/desktop");
        window.removeEventListener(APPLICATION_EVENTS.RESIZE, this.resizeBoundries);
    }
    ...
    checkOverlap = () => {
        ...
        if (rect.x.toFixed(1) < 50) { // if this window overlapps with SideBar
            this.props.hideSideBar(this.id, APP_CONSTANTS.BOOLEAN.TRUE);
        }
        else {
            this.props.hideSideBar(this.id, APP_CONSTANTS.BOOLEAN.FALSE);
        }
    }
    ...
    restoreWindow = () => {
        ...
        setTimeout(() => {
            this.setState({ maximized: APP_CONSTANTS.BOOLEAN.FALSE });
            this.checkOverlap();
        }, APP_CONSTANTS.TIME_OUTS.MS300);
    }
    ...
    maximizeWindow = () => {
        if (this.state.maximized) {
            ...
            this.setState({ maximized: APP_CONSTANTS.BOOLEAN.TRUE, height: 96.3, width: 100.2 });
            this.props.hideSideBar(this.id, APP_CONSTANTS.BOOLEAN.TRUE);
        }
    }
    ...
    closeWindow = () => {
        this.setWinowsPosition();
        this.setState({ closed: APP_CONSTANTS.BOOLEAN.TRUE }, () => {
            this.props.hideSideBar(this.id, APP_CONSTANTS.BOOLEAN.FALSE);
            setTimeout(() => {
                this.props.closed(this.id)
            }, APP_CONSTANTS.TIME_OUTS.MS300) // after 300ms this window will be unmounted from parent (Desktop)
        });
    }
    ...
    render() {
        return (
            <Draggable
                ...
                allowAnyClick={APP_CONSTANTS.BOOLEAN.FALSE}
...
export class WindowMainScreen extends Component {
    constructor() {
        super();
        this.state = {
            setDarkBg: APP_CONSTANTS.BOOLEAN.FALSE,
        }
    }
    componentDidMount() {
        setTimeout(() => {
            this.setState({ setDarkBg: APP_CONSTANTS.BOOLEAN.TRUE });
        }, APP_CONSTANTS.TIME_OUTS.SECONDS_3);
    }
    render() {
        return (
