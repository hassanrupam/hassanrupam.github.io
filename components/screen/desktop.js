...
import { SETTING_PAGES } from '../settings/constants';
...
export class Desktop extends Component {
    constructor() {
        ...
        this.app_stack = [];
        this.initFavourite = {};
        this.allWindowClosed = APP_CONSTANTS.BOOLEAN.FALSE;
        this.state = {
            focused_windows: {},
            closed_windows: {},
            allAppsView: APP_CONSTANTS.BOOLEAN.FALSE,
            overlapped_windows: {},
            disabled_apps: {},
            favourite_apps: {},
            hideSideBar: APP_CONSTANTS.BOOLEAN.TRUE,
            minimized_windows: {},
            desktop_apps: [],
            context_menus: {
                desktop: APP_CONSTANTS.BOOLEAN.FALSE,
                default: APP_CONSTANTS.BOOLEAN.FALSE,
            },
            showNameBar: APP_CONSTANTS.BOOLEAN.FALSE,
        }
    }
    ...
    checkForNewFolders = () => {
        var new_folders = secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.NEW_FOLDERS);
        if (new_folders === null && new_folders !== undefined) {
            secureLocalStorage.setItem(SECURE_STORAGE_STORE_KEY.NEW_FOLDERS, JSON.stringify([]));
        }
        else {
    ...
    setEventListeners = () => {
        document.getElementById("open-settings").addEventListener(APPLICATION_EVENTS.CLICK, () => {
    ...
    setContextListeners = () => {
        document.addEventListener(APPLICATION_EVENTS.CONTEXT_MENU, this.checkContextMenu);
        // on click, anywhere, hide all menus
        document.addEventListener(APPLICATION_EVENTS.CLICK, this.hideAllContextMenu);
    }

    removeContextListeners = () => {
        document.removeEventListener(APPLICATION_EVENTS.CONTEXT_MENU, this.checkContextMenu);
        document.removeEventListener(APPLICATION_EVENTS.CLICK, this.hideAllContextMenu);
    }
    ...
    showContextMenu = (e, menuName /* context menu name */) => {
        ...
        this.setState({ context_menus: { ...this.state.context_menus, [menuName]: APP_CONSTANTS.BOOLEAN.TRUE } });
    }

    hideAllContextMenu = () => {
        let menus = this.state.context_menus;
        Object.keys(menus).forEach(key => {
            menus[key] = APP_CONSTANTS.BOOLEAN.FALSE;
        });
    ...
    fetchAppsData = () => {
        ...
        apps.forEach((app) => {
            focused_windows = {
                ...focused_windows,
                [app.id]: APP_CONSTANTS.BOOLEAN.FALSE,
            };
            closed_windows = {
                ...closed_windows,
                [app.id]: APP_CONSTANTS.BOOLEAN.TRUE,
            };
            disabled_apps = {
                ...disabled_apps,
                [app.id]: app.disabled,
    ...
    updateAppsData = () => {
        ...
        apps.forEach((app) => {
            focused_windows = {
                ...focused_windows,
                [app.id]: ((this.state.focused_windows[app.id] !== undefined || this.state.focused_windows[app.id] !== null) ? this.state.focused_windows[app.id] : APP_CONSTANTS.BOOLEAN.FALSE),
            };
            minimized_windows = {
                ...minimized_windows,
                [app.id]: ((this.state.minimized_windows[app.id] !== undefined || this.state.minimized_windows[app.id] !== null) ? this.state.minimized_windows[app.id] : APP_CONSTANTS.BOOLEAN.FALSE)
            };
            disabled_apps = {
                ...disabled_apps,
                [app.id]: app.disabled
            ...
            closed_windows = {
                ...closed_windows,
                [app.id]: ((this.state.closed_windows[app.id] !== undefined || this.state.closed_windows[app.id] !== null) ? this.state.closed_windows[app.id] : APP_CONSTANTS.BOOLEAN.TRUE)
            };
            favourite_apps = {
                ...favourite_apps,
                [app.id]: app.favourite
    ...
    renderWindows = () => {
        let windowsJsx = [];
        apps.forEach((app, index) => {
            if (this.state.closed_windows[app.id] === APP_CONSTANTS.BOOLEAN.FALSE) {
    ...
    hideSideBar = (objId, hide) => {
        ...
        if (objId === null) {
            if (hide === APP_CONSTANTS.BOOLEAN.FALSE) {
                this.setState({ hideSideBar: APP_CONSTANTS.BOOLEAN.FALSE });
            }
            else {
                for (const key in this.state.overlapped_windows) {
                    if (this.state.overlapped_windows[key]) {
                        this.setState({ hideSideBar: APP_CONSTANTS.BOOLEAN.TRUE });
                        return;
    ...
    hasMinimised = (objId) => {
        ...
        minimized_windows[objId] = APP_CONSTANTS.BOOLEAN.TRUE;
        focused_windows[objId] = APP_CONSTANTS.BOOLEAN.FALSE;
        this.setState({ minimized_windows, focused_windows });

        this.hideSideBar(null, APP_CONSTANTS.BOOLEAN.FALSE);
    ...
    checkAllMinimised = () => {
        let result = APP_CONSTANTS.BOOLEAN.TRUE;
    ...
    openApp = (objId) => {
        ...
        if (this.app_stack.includes(objId)) this.focus(objId);
            ...
            var frequentApps = secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.FREQUENT_APPS) ? JSON.parse(secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.FREQUENT_APPS)) : [];
            ...
            secureLocalStorage.setItem(SECURE_STORAGE_STORE_KEY.FREQUENT_APPS, JSON.stringify(frequentApps));

            setTimeout(() => {
                favourite_apps[objId] = APP_CONSTANTS.BOOLEAN.TRUE; // adds opened app to sideBar
                closed_windows[objId] = APP_CONSTANTS.BOOLEAN.FALSE; // openes app's window
                this.setState({ closed_windows, favourite_apps, allAppsView: APP_CONSTANTS.BOOLEAN.FALSE }, this.focus(objId));
                this.app_stack.push(objId);
            }, APP_CONSTANTS.TIME_OUTS.MS200);
        }
    }
    ...
    closeApp = (objId) => {
        ...
        if (this.initFavourite[objId] === APP_CONSTANTS.BOOLEAN.FALSE) favourite_apps[objId] = APP_CONSTANTS.BOOLEAN.FALSE; // if user default app is not favourite, remove from sidebar
        closed_windows[objId] = APP_CONSTANTS.BOOLEAN.TRUE; // closes the app's window
    ...
    focus = (objId) => {
        ...
        var focused_windows = this.state.focused_windows;
        focused_windows[objId] = APP_CONSTANTS.BOOLEAN.TRUE;
        for (let key in focused_windows) {
            if (focused_windows.hasOwnProperty(key)) {
                if (key !== objId) {
                    focused_windows[key] = APP_CONSTANTS.BOOLEAN.FALSE;
                }
            }
        }
    ...
    addNewFolder = () => {
        this.setState({ showNameBar: APP_CONSTANTS.BOOLEAN.TRUE });
    }
    ...
    addToDesktop = (folder_name) => {
        ...
        var new_folders = JSON.parse(secureLocalStorage.getItem(SECURE_STORAGE_STORE_KEY.NEW_FOLDERS));
        new_folders.push({ id: `new-folder-${folder_id}`, name: folder_name });
        secureLocalStorage.setItem(SECURE_STORAGE_STORE_KEY.NEW_FOLDERS, JSON.stringify(new_folders));

        this.setState({ showNameBar: APP_CONSTANTS.BOOLEAN.FALSE }, this.updateAppsData);
    }
    ...
    renderNameBar = () => {
        ...
        let removeCard = () => {
            this.setState({ showNameBar: APP_CONSTANTS.BOOLEAN.FALSE });
        }
        ...
        return (
            <div className="absolute rounded-md top-1/2 left-1/2 text-center text-white font-light text-sm bg-ub-cool-grey transform -translate-y-1/2 -translate-x-1/2 sm:w-96 w-3/4 z-50">
                <div className="w-full flex flex-col justify-around items-start pl-6 pb-8 pt-6">
                    ...
                    <input className="outline-none mt-5 px-1 w-10/12  context-menu-bg border-2 border-yellow-700 rounded py-0.5" id="folder-name-input" type="text" autoComplete="off" spellCheck="APP_CONSTANTS.BOOLEAN.FALSE" autoFocus={APP_CONSTANTS.BOOLEAN.TRUE} />
                </div>
                <div className="flex">
